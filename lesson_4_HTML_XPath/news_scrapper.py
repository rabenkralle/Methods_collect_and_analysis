...
class Scrapper():       #Создаем класс для сбора новостей с сайтов
    ...
    def parse_lenta_ru(cls):                #Собираем новости с сайта Лента.ру
        ...
        data = {}                           #Не получилось собрать первую новость вместе с остальными, потому добавляем отдельно
        data['Title'] = dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//h2/a/text()')[0].replace('\xa0', ' ')
        data['Link'] = lentaru_link + dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//h2/a/@href')[0]
        data['Source'] = 'Lenta.ru'
        data['Time'] = dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//h2/a/time/text()')[0]
        news = []
        news.append(data)
        text = dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//div[@class = "item"]/a/text()')
        link = dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//div[@class = "item"]/a/@href')
        time = dom.xpath('//div/section[contains(@class,"b-top7-for-main")]//div[@class = "item"]/a/time/@datetime')
        for i in range(len(text)):
            data = {}
